...
Describe 'ConvertTo-RomanNumeral' -Tag ConvertTo-RomanNumeral {
	...
	Context 'Convert a number to a Roman numeral' -Tag Convert,ConvertTo,ConvertToRomanNumeral,RomanNumeral {
		It "Should convert '<Value>' to '<Result>' (ASCII)" -TestCases @(
			...
			ConvertTo-RomanNumeral $Value |Should -BeExactly $Result -Because 'parameter should work'
			$Value |ConvertTo-RomanNumeral |Should -BeExactly $Result -Because 'pipeline should work'
		}
		It "Should convert '<Value>' to '<Result>' (Unicode)" -TestCases @(
			...
			Param([int] $Value, [string] $Result)
			ConvertTo-RomanNumeral $Value -Unicode |Should -BeExactly $Result -Because 'parameter should work'
			$Value |ConvertTo-RomanNumeral -Unicode |Should -BeExactly $Result -Because 'pipeline should work'
		}
	}
}
AfterAll {
