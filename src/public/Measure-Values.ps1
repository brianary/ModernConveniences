...
.EXAMPLE
1..54 |ForEach-Object {Get-Random} |Measure-Values
...
.EXAMPLE
1..127 |ForEach-Object {Get-Date -UnixTimeSeconds (Get-Random)} |Measure-Values
...
End
{
	foreach($type in $values.Keys)
	{
		...
		elseif($type -in [datetime],[dateonly],[timeonly],[datetimeoffset])
		{
			...
			[Collections.ArrayList] $e = $v |ConvertTo-EpochTime
