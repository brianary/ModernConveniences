...
.EXAMPLE
Format-EscapedUrl -Clipboard

Updates the URL on the clipboard with a more aggressively escaped version.

.EXAMPLE
Format-EscapedUrl "https://example.com/search(en-US)?q=Name%20%3D%20'System'&sort=y"
...
Process
{
    if($Clipboard) {Get-Clipboard |Format-EscapedUrl |Set-Clipboard}
    else
    {
        foreach($u in $Uri)
        {
