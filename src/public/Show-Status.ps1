...
.EXAMPLE
Show-Status UserName HomeDirectory -Separator ' * '
...
Begin
{
    ...
    {
        Set-Variable -Name LEFTEND -Value (Get-Unicode 0xE0B6) -Scope Script -Option Constant -EA Ignore `
            -Description 'nf-ple-left_half_circle_thick (wt)'
        Set-Variable -Name RIGHTEND -Value (Get-Unicode 0xE0B4) -Scope Script -Option Constant -EA Ignore `
            -Description 'nf-ple-right_half_circle_thick (wt)'
        Set-Variable -Name DOTNET -Value (Get-Unicode 0xE77F) -Scope Script -Option Constant -EA Ignore `
            -Description 'nf-dev-dotnet (wt)'
        Set-Variable -Name GIT -Value (Get-Unicode 0xF1D2) -Scope Script -Option Constant -EA Ignore `
            -Description 'nf-fa-git_square (wt)'
        Set-Variable -Name WINDOWS -Value (Get-Unicode 0xF17A) -Scope Script -Option Constant -EA Ignore `
            -Description 'nf-fa-windows (wt)'
        Set-Variable -Name LINUX -Value (Get-Unicode 0xF17C) -Scope Script -Option Constant -EA Ignore `
            -Description 'nf-fa-linux (wt)'
        Set-Variable -Name MAC -Value (Get-Unicode 0xF179) -Scope Script -Option Constant -EA Ignore `
            -Description 'nf-fa-apple (wt)'
        Set-Variable -Name POWERSHELL -Value (Get-Unicode 0xE86C) -Scope Script -Option Constant -EA Ignore `
    ...
    {{
        if(Get-OutdatedModules |Select-Object -First 1) {'psmodules'}
    }}
    ...
    filter Format-Status
    {
        ...
        switch($Status)
        {
            AdminIndicator { (Test-Administrator) ? $REDCIRCLE : $BLUECIRCLE }
            ...
            DriveUsage
            {
                $HARDDISK + ' ' + ((Get-PSDrive -PSProvider FileSystem |Where-Object {$null -ne $_.Free} |
                    ForEach-Object {"$(Get-Unicode (0x1F311 + [math]::Round((5*$_.Used)/($_.Used+$_.Free)) ))$($_.Name)"}) -join ' ')
            }
            ExplorerUser
            {
            ...
            {
                $updates = @(
                    (Invoke-CachedCommand (Get-ChocoTest) -ExpiresAfter 20:00 -Force:$Force)
                    (Invoke-CachedCommand (Get-WingetTest) -ExpiresAfter 20:00 -Force:$Force)
                    (Invoke-CachedCommand (Get-NpmTest) -ExpiresAfter 20:00 -Force:$Force)
                    (Invoke-CachedCommand (Get-PSModulesTest) -ExpiresAfter 20:00 -Force:$Force)
                ) |Where-Object {$_}
...
Process
{
	Write-Info $LEFTEND -ForegroundColor $BackgroundColor -NoNewLine
	Write-Info " $WAIT " -ForegroundColor $ForegroundColor -BackgroundColor $BackgroundColor -NoNewline
	Write-Info "$RIGHTEND`r$LEFTEND" -ForegroundColor $BackgroundColor -NoNewLine
	Write-Info (($Status |Format-Status) -join $Separator) `
	...
	Write-Info $RIGHTEND -ForegroundColor $BackgroundColor
}
