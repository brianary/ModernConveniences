...
Begin
{
    ...
    else {Import-CharConstants 'full block' 'light shade'; $usedchar, $freechar = ${full block}, ${light shade}}
    filter Show-DriveUsage
    {
        ...
        $usedpercent, $freepercent, $size, $free, $used, $usedgraph, $freegraph =
            ...
            ($TotalSize |Format-ByteUnits -Precision 0),
            ($AvailableFreeSpace |Format-ByteUnits -Precision 0),
            ($usedspace |Format-ByteUnits -Precision 0),
        ...
        Write-Info $usedgraph -fg Cyan -NoNewLine
        Write-Info $freegraph -fg DarkCyan
        $fqdn = $Name -ne $VolumeLabel ? "$Name $VolumeLabel [$DriveFormat]" : "$Name [$DriveFormat]"
        Write-Info "$fqdn $size = $free ($freepercent%) free + $used ($usedpercent%) used" -fg $color
    }
}
Process
{
