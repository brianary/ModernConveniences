...
.EXAMPLE
Get-PSDrive |Measure-Properties |Format-Table -AutoSize
...
Process
{
    ...
    foreach($property in $InputObject.PSObject.Properties)
    {
        ...
        elseif($value.Contains($propname))
        {
            ...
            {
                Stop-ThrowError "Property '$propname' has conflicting types: '$($type[$propname])' vs '$proptype'" -Argument InputObject
            }
...
End
{
    Write-Info 'Type: ' -fg Green -NoNewLine
    Write-Info ($typename.GetEnumerator() -join ', ') -fg Gray
    Write-Info 'Count: ' -fg Green -NoNewLine
    Write-Info $count -fg Gray
