...
.EXAMPLE
Get-FrenchRepublicanDate 2020-07-08
...
{ # 1792-09-22 = 1 Vendémiaire Ⅰ
	if($Date -lt $origin) {Stop-ThrowError ('Dates prior to {0:MMMM d, yyyy} are not yet supported.' -f $origin) -NotImplemented}
	...
	if(${jour de l'année} -gt 364)
	{
		...
		[int] ${past leap days}, [int] ${last year length} = switch($Method)
		{
			...
			Equinox {Stop-ThrowError 'Equinox isn''t supported yet' -NotImplemented}
	...
	[pscustomobject]@{
		Year = ${l'année}
		Année = ConvertTo-RomanNumeral ${l'année}
		AnnéeUnicode = ConvertTo-RomanNumeral ${l'année} -Unicode
		Month = $mois
