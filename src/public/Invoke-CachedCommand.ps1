...
.EXAMPLE
Invoke-CachedCommand {Invoke-RestMethod https://example.org/endpoint} -Session
...
Begin
{
    ...
    function Get-ScriptBlockHash
    {
        ...
        return (@("$ScriptBlock") + @($BlockArgs |ForEach-Object {ConvertTo-Json $_ -Compress}) |
            ForEach-Object {ConvertTo-Base64 $Script:Hash.ComputeHash([Text.Encoding]::UTF8.GetBytes($_)) -UriStyle}) -join '.'
    }
