...
.EXAMPLE
ImportIcal\Import-Calendar -String (Invoke-RestMethod https://spokanefilmfestival.org/events/?ical=1) |Select-Object -ExpandProperty Events |Convert-ICalToMermaidGantt
...
Process
{
    ...
    elseif($Start -isnot [datetime]) {Stop-ThrowError 'Start was not a DateTime' -Argument Start}
    ...
    elseif($End -isnot [datetime]) {Stop-ThrowError 'End was not a DateTime' -Argument 'End'}
