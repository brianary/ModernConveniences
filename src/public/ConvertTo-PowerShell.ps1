...
.EXAMPLE
4096LMB |ConvertTo-PowerShell

4LGB

.EXAMPLE
ConvertFrom-Json '[{"a":1,"b":2,"c":{"d":"\/Date(1490216371478)\/","e":null}}]' |ConvertTo-PowerShell
...
Begin
{
	...
	$Local:PSDefaultParameterValues = @{
		...
		'ConvertTo-PowerShell:Width'    = $Width
	}
	...
	if($GenerateKey)
	{
		[byte[]] $KeyBytes = Get-RandomBytes 32
	...
	if($KeyBytes)
	{
		$Local:PSDefaultParameterValues['ConvertTo-PowerShell:Key'] = $KeyBytes
		$keyopt = ' -Key $key'
		$dpapiwarn = ''
	...
	filter Format-ParameterAttribute
	{
		...
		switch($name)
		{
			...
			Alias {"[Alias($(($AliasNames |ConvertTo-PowerShell -SkipInitialIndent) -join ','))]"}
			ValidateCount {"[$name($MinLength,$MaxLength)]"}
			ValidateDrive {"[$name($(($ValidRootDrives |ConvertTo-PowerShell -SkipInitialIndent) -join ','))]"}
			...
			ValidateSet {"[$name($(($ValidValues |ConvertTo-PowerShell -SkipInitialIndent) -join ','))]"}
			default {"[$name()]"}
		}
	}
...
"@}
	...
	{@"
${itab}Param(
$(($Value.GetEnumerator() |ForEach-Object {"$tabtab$(ConvertTo-PowerShell $_.Value)"}) -join ',')
$tab)
"@}
