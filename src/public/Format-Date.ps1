...
Process
{
	switch($Format)
	{
		...
		Iso8601Week
		{
			...
			if(1 -eq (Get-Date $Date -uf %V) -and $Date.Month -eq 12) {"$($Date.Year+1)-W$V"}
			elseif(50 -lt (Get-Date $Date -uf %V) -and $Date.Month -eq 1) {"$($Date.Year-1)-W$V"}
		...
		Iso8601WeekDate
		{
			...
			if(1 -eq (Get-Date $Date -uf %V) -and $Date.Month -eq 12) {"$($Date.Year+1)-W$V-$w"}
			elseif(50 -lt (Get-Date $Date -uf %V) -and $Date.Month -eq 1) {"$($Date.Year-1)-W$V-$w"}
