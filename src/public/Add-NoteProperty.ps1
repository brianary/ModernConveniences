...
.EXAMPLE
Get-ChildItem Get-*.ps1 |Add-NoteProperty Size {Format-ByteUnits $Length -Precision 1} -Properties Length -PassThru |Format-Table Size,Name -AutoSize
...
.EXAMPLE
Get-ChildItem Get-*.ps1 |Add-NoteProperty @{Size={Format-ByteUnits $_.Length -Precision 1};Ext={$_ |Split-Path -Extension};Hat='Y'} -PassThru |Format-Table Size,Name,Ext,Hat -AutoSize
